...
Let the monolithic application be represented as a directed attributed graph $G = (V, E, X)$, where $V$ is the set of classes, $E$ represents structural dependencies, and $X$ is the semantic feature matrix.
...
The clustering algorithm then partitions $V$ into $K$ subsets $C = \{c_1, c_2, \dots, c_K\}$ by maximizing the Structural Modularity ($SM$) and Semantic Cohesion ($SC$):
\begin{equation}
...
\begin{itemize}
    ...
    \item \textbf{JPetStore:} A standard monolithic web application demonstrating standard e-commerce workflows.
